	...
	return (
		<InspectorControls >
			...
			>
				...
				<ButtonGroup aria-label={ __( 'Adaptive Height', 'eedee-gutenslider' ) } className="gutenslider-toggle-adaptive-height">
					<Button
						isSecondary={ adaptiveHeight === false }
						isPrimary={ adaptiveHeight === true }
						aria-pressed={ adaptiveHeight === true }
						onClick={ () => {
							block.setAttributes( { adaptiveHeight: true } );
						} }
					>
						Adaptive Height
					</Button>
					<Button
						isSecondary={ adaptiveHeight === true }
						isPrimary={ adaptiveHeight === false }
						aria-pressed={ adaptiveHeight === false }
						onClick={ () => {
							block.setAttributes( { adaptiveHeight: false } );
						} }
					>
						Fixed Height
					</Button>
				</ButtonGroup>
				{ ! isFullScreen && ! adaptiveHeight &&
					<ResponsiveTabs
						breakpoints={ responsiveSliderHeightBreakpoints }
					>
						<UnitButton
							units={ [
								{ value: 'vh', label: '%', default: 0, min: 20, max: 100 },
								{ value: 'px', label: 'px', default: 10, min: 100, max: 1500 },
							] }
							step={ 1 }
							transformUnits={ true }
						/>
			...
			>
				...
				/>
				<BaseControl>
					<BaseControl.VisualLabel>{ __( 'Arrow Color', 'eedee-media-slider' ) }</BaseControl.VisualLabel>
					<ColorPalette
						value={ arrowColor }
						colors={ [
